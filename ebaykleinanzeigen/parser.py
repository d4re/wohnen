...
def parse(html_input):
    ...
    for flat in all_flats:
        ...
        flat_dict['addr'] = top.xpath(".//div[contains(@class,'aditem-main--top--left')]")[0].text_content().strip()
        ...
        flat_dict['date_found'] = top.xpath(".//div[contains(@class,'aditem-main--top--right')]")[0].text_content().strip()
        if len(flat_dict['date_found']) > 0:
          flat_dict['date_found'] = flat_dict['date_found'].replace('Heute,', datetime.datetime.now().strftime('%d.%m.%Y'))
          flat_dict['date_found'] = flat_dict['date_found'].replace('Gestern,', (datetime.date.today() - datetime.timedelta(days=1)).strftime('%d.%m.%Y'))
        else:
        ...
        flat_dict['properties']['Miete'] = middle.xpath(".//p[contains(@class,'aditem-main--middle--price')]")[0].text_content()
        flat_dict['properties']['Miete'] = re.sub('\n\s+','', flat_dict['properties']['Miete'])
        flat_dict['properties']['Beschreibung'] = middle.xpath(".//p[contains(@class,'aditem-main--middle--description')]")[0].text_content()
