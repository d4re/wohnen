...
def parse(html_input):
    ...
    for flat in all_flats:
        ...
        flat_dict[
            "addr"
        ] = f"{flat['address']['street']} {flat['address']['houseNumber']}, {flat['address']['zip']} {flat['address']['city']}"
        flat_dict["plz"] = flat['address']['zip']
        flat_dict["kiez"] = ""
        ...
        if "images" in flat and len(flat["images"]) > 0:
            flat_dict["image"] = quote(
                urljoin(base_url, flat["images"][0]["filePath"]), safe=":/"
            )

        flat_dict["date_found"] = datetime.datetime.strptime(
            flat["date"], "%Y-%m-%d %H:%M:%S"
        ).strftime("%d.%m.%Y %H:%M")
